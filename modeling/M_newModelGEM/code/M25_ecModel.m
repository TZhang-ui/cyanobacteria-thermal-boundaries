...
newKcat = readmatrix('M25_kcat.tsv', 'FileType', 'text');
...
saveEcModel(ecModel,'Mtaihu98_T25_ecModel.yml' ); % Update filename if necessary

%% Reload EC model after modifications
ecModel = loadEcModel('Mtaihu98_T25_ecModel.yml');

%% Apply kcat constraints to stoichiometric matrix
ecModel = applyKcatConstraints(ecModel);

%% Set protein pool upper bound
% Default upper limit: 0.47 g/gDW (model-defined)
ecModel = setProtPoolSize(ecModel, 0.47, 0.5, 0.5);
...
writetable(fluxTable, 'M25_GECKO_light_Flux.csv');
